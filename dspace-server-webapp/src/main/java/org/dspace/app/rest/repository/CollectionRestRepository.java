...
public class CollectionRestRepository extends DSpaceObjectRestRepository<Collection, CollectionRest> {
    ...
    @SearchRestMethod(name = "findCollectionByNameAndCommunityID")
    public CollectionRest findCollectionByNameAndCommunityID(@Parameter(value = "communityid", required = true) UUID communityid,
                                                             @Parameter(value = "collectionName", required = true) String collectionName) {
        try {
            Context context = obtainContext();
            //
            List<Community> communities = new ArrayList<>();
            System.out.println("collectionName aname::::"+collectionName);
            System.out.println("communityid ::::"+communityid);

            Community community= communityService.find(context,communityid);
            Collection collection=null;
            if(community != null){
                collection= cs.findCollectionByNameAndCommunityID(context,community,collectionName );

                if(collection == null){
                    try {

                        collection = cs.create(context, community);
                        System.out.println("collection ID::::"+collection.getID());

                        cs.setMetadataSingleValue(context,collection,"dc","title",null,null,collectionName);
                        cs.update(context, collection);
                        System.out.println("collection ID::::"+collection.getName());
                        context.commit();
                    } catch (SQLException e) {
            ...
            System.out.println("collection found...."+collection.getName());
            return converter.toRest(collection, utils.obtainProjection());
        } catch (SQLException e) {
            e.printStackTrace();
            throw new RuntimeException(e.getMessage(), e);

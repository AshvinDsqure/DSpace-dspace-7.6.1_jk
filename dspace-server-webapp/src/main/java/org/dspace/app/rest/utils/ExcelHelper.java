    ...
    public static ByteArrayInputStream tutorialsToExcel(List<ExcelDTO> tutorials) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            for (ExcelDTO item : tutorials) {
                ...
                row.createCell(1).setCellValue(item.getBundlename());
                row.createCell(2).setCellValue(item.getCaseDetail());
                row.createCell(3).setCellValue(item.getNoOfPages());
                row.createCell(4).setCellValue(item.getNature());
                row.createCell(5).setCellValue(DateFormateddmmyyyy(item.getUploaddate()));
                row.createCell(6).setCellValue(item.getUploadedby());
                i++;
            }

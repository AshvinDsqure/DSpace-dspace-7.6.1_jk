...
public class CommunityDAOImpl extends AbstractHibernateDSODAO<Community> implements CommunityDAO {
    ...
    @Override
    public Community searchByTitle(Context context, MetadataField metadataField, String title) throws Exception {
        System.out.println("::titile::"+title);
        Query query = createQuery(context, "SELECT community FROM Community as community join community.metadata  metadatavalue  WHERE   metadatavalue.metadataField = :metadataField AND lower(STR(metadatavalue.value)) = :title");

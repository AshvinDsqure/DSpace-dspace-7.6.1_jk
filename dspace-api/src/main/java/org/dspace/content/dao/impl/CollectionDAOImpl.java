...
public class CollectionDAOImpl extends AbstractHibernateDSODAO<Collection> implements CollectionDAO {
    ...
    @Override
    public Collection findCollectionByNameAndCommunityID(Context context, Community community, String collectionName) throws SQLException {
        System.out.println("collectionName:::"+collectionName);
        String q = "select c.* as id from collection as c Inner join community2collection c2c ON c2c.collection_id=c.uuid and c2c.community_id ='"+community.getID()+"' Inner join metadatavalue as m on m.dspace_object_id=c.uuid Inner join metadatafieldregistry as mr ON mr.metadata_field_id = m.metadata_field_id where mr.metadata_field_id=73 and m.text_value= '"+collectionName+"'";
        //select c.* as id from collection as c Inner join community2collection c2c ON c2c.collection_id=c.uuid and c2c.community_id ='346f492c-20be-4350-ad49-a6b38e51cc4d' Inner join metadatavalue as m on m.dspace_object_id=c.uuid Inner join metadatafieldregistry as mr ON mr.metadata_field_id = m.metadata_field_id where mr.metadata_field_id=71 and m.text_value= '2015'
        Collection collection=null;
        try {
            collection=  (Collection) getHibernateSession(context).createSQLQuery(q).addEntity(Collection.class).getSingleResult();
        }catch (Exception e){
            //e.printStackTrace();
        }
        return collection;

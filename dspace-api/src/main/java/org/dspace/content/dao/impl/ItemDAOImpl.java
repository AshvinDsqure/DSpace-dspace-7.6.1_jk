...
public class ItemDAOImpl extends AbstractHibernateDSODAO<Item> implements ItemDAO {
    ...
    @Override
    public Item findByCIno(Context context,String cino) throws SQLException{
        String q = "SELECT item FROM Item as item WHERE item.inArchive=:in_archive AND cino = :cino";
    ...
    @Override
    public Item findItemByBitstream(Context context, Bitstream bitstreamid) throws SQLException{
        String q = "SELECT item FROM Item as item join item.bundles bundles join bundles.bitstreams  bundles2bitstreams WHERE bundles2bitstreams=:bitstreamid ";
        Query query = createQuery(context, q);

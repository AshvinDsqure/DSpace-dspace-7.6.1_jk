...
@Table(name = "civil_t",schema = "public")
public class Civilt extends DSpaceObjectCIS {
    @Id
    ...
    @Where(clause = "policeStnT.police_st_code != 0")
    private CriminalT criminalT = null;
    @OneToOne(fetch = FetchType.LAZY,cascade = {CascadeType.ALL})
    @JoinColumn(name = "cino", referencedColumnName = "cino")
    private DspaceToCisMapping dspaceToCisMapping;


    @Column(name="judge_code",columnDefinition = "bpchar")
    protected String judge_code;
  ...
  @OneToMany(mappedBy = "civilt", cascade = CascadeType.ALL)
    private List<ExtraActT> extraactt;
    @OneToMany(mappedBy = "civilt", cascade = CascadeType.ALL,fetch = FetchType.LAZY)
    @Where(clause = "type = 2 and  display = 'Y'" )
    private List<Civaddresst> extrarespondate;

    @OneToMany(mappedBy = "civilt", cascade = CascadeType.ALL,fetch = FetchType.LAZY)
    @Where(clause = "type = 1 and  display = 'Y'" )
    private List<Civaddresst> extrapetitioner;
    ...
    //added ashvin
    @Column(name="ci_cri",columnDefinition = "smallint")
    protected Short ci_cri;
   ...
   /* @Column(name="newregcase_type",columnDefinition = "smallint")
    ...
    public List<ExtraActT> getExtraactt() {
        return extraactt;
    }

    public void setExtraactt(List<ExtraActT> extraactt) {
        this.extraactt = extraactt;
    }
    ...
    public DspaceToCisMapping getDspaceToCisMapping() {
        return dspaceToCisMapping;
    }

    public void setDspaceToCisMapping(DspaceToCisMapping dspaceToCisMapping) {
        this.dspaceToCisMapping = dspaceToCisMapping;
    }

    public CriminalT getCriminalT() {
        return criminalT;
    }

    public void setCriminalT(CriminalT criminalT) {
        this.criminalT = criminalT;
    }

    public Short getCi_cri() {
        return ci_cri;
    }

    public void setCi_cri(Short ci_cri) {
        this.ci_cri = ci_cri;
    }
    ...
    public List<Civaddresst> getExtrarespondate() {
        return extrarespondate;
    }

    public void setExtrarespondate(List<Civaddresst> extrarespondate) {
        this.extrarespondate = extrarespondate;
    }

    public List<Civaddresst> getExtrapetitioner() {
        return extrapetitioner;
    }

    public void setExtrapetitioner(List<Civaddresst> extrapetitioner) {
